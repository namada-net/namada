...
#[derive(Clone, PartialEq)]
pub enum FeeAuhtorization {
    // FIXME: probably PartialEq for this should only care about the pubkey
    // field and ignore the boolean
    /// A wrapper signer
    Signer {
        /// The wrapper signer
        pubkey: common::PublicKey,
        /// Flag to mark a disposable fee payer
        disposable_fee_payer: bool,
    },
    /// A serialized signature
    Signature(Vec<u8>),
}

impl FeeAuhtorization {
    /// Returns the fee payer's public key if provided, otherwise returns an
    /// error.
    // FIXME: better to return an Option instead? Maybe yes
    pub fn fee_payer_or_err(&self) -> Result<&common::PublicKey, Error> {
        match self {
            FeeAuhtorization::Signer { pubkey, .. } => Ok(pubkey),
            FeeAuhtorization::Signature(_) => Err(Error::Other(
                "Expected wrapper signer, found wrapper signature".to_string(),
            )),
        }
    }

    // FIXME: move this method to SigningWrapperData
    /// Returns whether fees will be paid from a disposable address
    pub fn disposable_fee_payer(&self) -> bool {
        match self {
            FeeAuhtorization::Signer {
                pubkey: _,
                disposable_fee_payer,
            } => *disposable_fee_payer,
            FeeAuhtorization::Signature(_) => false,
        }
    }
}
...
pub struct SigningWrapperData {
    // FIXME: use a set here to avoid duplicates
    /// The signing data for each one of the inner transactions of this batch
    pub signing_data: Vec<SigningTxData>,
    // FIXME: in the cli we have to ensure we have a wrapper signature before
    // submitting the transaction. Could also be done by just making the cli
    // args mandatory (which probably already is) FIXME: actually if we
    // produced the wrapper signature offline then I think we don't need the
    // above vector, we can just create the Authorization section and attach it
    // to the tx FIXME: maybe rename to fee_auth
    /// The signing data for the wrapper itself
    pub fee_payer: FeeAuhtorization,
}
...
impl SigningData {
    fn into_components(self) -> (Vec<SigningTxData>, Option<FeeAuhtorization>) {
        match self {
            ...
            SigningData::Wrapper(signing_wrapper_data) => (
                ...
                Some(signing_wrapper_data.fee_payer),
            ),
        }
    }
...
/// Error.
///
/// Optionally sign the wrapper header separately.
///
...
{
    let (signing_data, fee_auth) = signing_data.into_components();
    for signing_tx_data in signing_data {
        ...
        // wallet
        if let Some(account_public_keys_map) =
        ...
        if used_pubkeys_len < signing_tx_data.threshold {
            return Err(Error::from(TxSubmitError::MissingSigningKeys(
                signing_tx_data.threshold,
                used_pubkeys_len,
            )));
        }
    }
    ...
    // Then try signing the wrapper header (fee payer) if requested. Check if
    // there's a provided wrapper signature, otherwise sign with the
    // software wallet or use the fallback
    match &fee_auth {
        Some(FeeAuhtorization::Signer {
            pubkey,
        ...
        Some(FeeAuhtorization::Signature(sig_bytes)) => {
...
pub async fn aux_signing_data(
    ...
    let fee_payer = match &wrapper_signature {
        Some(signature) => FeeAuhtorization::Signature(signature.to_owned()),
        None => match &args.wrapper_fee_payer {
            Some(pubkey) => FeeAuhtorization::Signer {
                pubkey: pubkey.clone(),
    ...
    let signing_inner_data = SigningTxData {
        owner,
        public_keys,
        threshold,
        account_public_keys_map,
    ...
    Ok(SigningWrapperData {
        signing_data: vec![signing_inner_data],
        fee_payer,
    })
}

/// Extends the signing data for the inner transactions with the necessary data
/// to generate a wrapper signature
pub async fn aux_wrapper_signing_data(
    // FIXME: instead of passing the args pass an either with the serialized
    // wrapper signature or the signer
    args: &args::Tx<SdkTypes>,
    wrapper_signature: Option<Vec<u8>>,
...
mod test_signing {
    ...
    async fn test_sign_tx_hw_failure() {
        ...
        let signing_data = SigningWrapperData {
            signing_data: vec![SigningTxData {
                owner: None,
            ...
            fee_payer: FeeAuhtorization::Signer {
                pubkey: public_key_fee.clone(),
                disposable_fee_payer: false,
            },
        };
