...
use masp_primitives::zip32::{
    ExtendedKey, PseudoExtendedKey, Scope as Zip32Scope,
};
...
pub async fn build_ibc_transfer(
    ...
    let data = if args.port_id == PortId::transfer() {
        let refund_target =
            get_ibc_refund_target(&args.source, args.refund_target.as_ref())?;
        let sender = refund_target.to_string().into();
    ...
    {
        let refund_transparent_addr =
            if matches!(&args.source, TransferSource::ExtendedKey(_)) {
                Some(TransferTarget::Address(
                    gen_ibc_transparent_refund_target(context).await?,
                ))
            } else {
                None
            };

        let refund_target = get_ibc_refund_target(
            &args.source,
            args.refund_target
                .as_ref()
                .or(refund_transparent_addr.as_ref()),
        )?;
        let sender = refund_target.to_string().into();
...
/// Generate a transparent IBC refund target
///
/// This is the legacy method of getting a refund target
/// for MASP unshielding transfers. Currently required by
/// NFT transfers.
async fn gen_ibc_transparent_refund_target(
    context: &impl Namada,
) -> Result<Address> {
    // Generate a new transparent address if it doesn't exist
    let mut rng = OsRng;
    let mut wallet = context.wallet_mut().await;
    let mut alias = format!("{IBC_REFUND_ALIAS_PREFIX}-{}", rng.next_u64());
    ...
    wallet
        ...
        .ok_or_else(|| {
            Error::Other("Adding a new refund address failed".to_string())
        })?;
    wallet
        .save()
        .map_err(|e| Error::Other(format!("Saving wallet error: {e}")))?;
...
/// Returns a valid IBC refund target
///
/// Generates a random payment address if the source is a spending key,
/// or takes the transparent source as a refund target. If the refund
/// target is overriden, an error may be emitted, in case the target
/// is a foreign chain account.
fn get_ibc_refund_target(
    source: &TransferSource,
    override_refund_target: Option<&TransferTarget>,
    ...
    match (source, override_refund_target) {
        ...
        (
            ...
            Some(
                ...
                | TransferTarget::PaymentAddress(_)),
            ),
        ) => {
        ...
        (_, Some(TransferTarget::Ibc(_))) => Err(Error::Other(
            "Can't specify foreign chain address as an IBC refund target"
                .to_string(),
        )),
        (TransferSource::ExtendedKey(esk), None) => {
            let ivk = esk
                .to_viewing_key()
                .to_diversifiable_full_viewing_key()
                .to_ivk(Zip32Scope::External);

            // Generate a random payment address
            let mut diversifier = Diversifier([0u8; 11]);
            loop {
                OsRng.fill_bytes(&mut diversifier.0);

                if let Some(pa) = ivk.to_payment_address(diversifier) {
                    break Ok(TransferTarget::PaymentAddress(pa.into()));
                }
            }
        }
        (TransferSource::Address(addr), None) => {
            Ok(TransferTarget::Address(addr.clone()))
        }
    }
}
